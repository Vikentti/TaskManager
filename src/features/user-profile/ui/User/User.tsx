...
import {useUser} from "@/features/user-profile/lib/hooks/useUser";
...
const User = ({className}: UserProps) => {
  ...
  // const {userName, userNameFirstLetter} = useUser()
  const userName = 'Kesha'
  const userNameFirstLetter = userName.slice(0, 1).toUpperCase()

  return (
    <div
      className={classNames(className, 'user', 'container')}
    >
      <div className="user__main">
        <div className="user__icon">{userNameFirstLetter}</div>
        <div className="user__welcome">
          <p className="user__welcome-text">Hello!</p>
          <p className="user__welcome-name">{userName}</p>
        </div>
      </div>
      <div className="user__nitification">
